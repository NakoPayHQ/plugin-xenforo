...
2. Upload the `NakoPay/BtcPay` folder into your XenForo installation's `src/addons/` directory so the final path is:

   ```
   <xenforo-root>/src/addons/NakoPay/BtcPay/
   ```
...
3. In XenForo admin go to **Add-ons -> Install/upgrade** and install **NakoPay Bitcoin Payments**.
   ...
   Or via CLI:
   ```bash
   php cmd.php xf:addon-install NakoPay/BtcPay
   ```
...
1. XenForo admin -> **Add-ons -> NakoPay Bitcoin Payments -> Uninstall**.
   ...
   Or via CLI:
   ```bash
   php cmd.php xf:addon-uninstall NakoPay/BtcPay
   ```
